  ...
  exports.getRandomEpisode = async function (show) {
    try {
      ...
      if (showReq.data.Response === 'False' ||  showReq.data.Type !== 'series') {
        return JSON.stringify({message: 'We were unable to find your show.',Response: 'False'}, null, 4)
      }
      ...
      const episodeReq = await axios.get('http://www.omdbapi.com', {
          params: {
            ...
            episode: await seasonReq.data.Episodes[await getRandomInt(0, await seasonReq.data.Episodes.length)].Episode
          }
      });
